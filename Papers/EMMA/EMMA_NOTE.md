...
红外与可见光图像融合结合了红外图像的热目标信息和可见光图像的纹理细节，提升场景感知能力。红外图像突出热辐射目标但缺乏细节，可见光图像纹理丰富但对光照敏感，简单融合易导致关键信息丢失或冗余。Current works only consider the use of single sensors, or simply combine different sensors, ignoring the fact that the importance of different sensors varies as the environment changes。针对以上问题，我们通过注意力分支学习红外图像与可见光图像的动态权重，使两种模态特征能根据当前环境状况进行差异化贡献。该权重学习模块与多模态特征提取分支进行深度融合，同时为了充分挖掘红外图像与可见光图像间的潜在关联，我们设计了一种融合注意力的双阶段融合策略，在已有EMMA框架下改进，引入轻量级高效backbone，替换传统的Unet，解决输入尺度不同问题，并且强化特征提取；改进了伪感知模块，在I图像和V图像分策略优化，V图像进行增强特征提取，I图像简单特征提取；融合模块设计了混合注意力机制，加强特征提取一种基于自监督学习的新型融合方法；实验表明，该方法在红外可见光图像融合任务上取得了良好的效果。
