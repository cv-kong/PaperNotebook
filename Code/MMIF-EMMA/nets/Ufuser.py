...
class Restormer_CNN_block(nn.Module):
    def __init__(self,in_dim,out_dim):
        ...
        self.embed=nn.Conv2d(in_dim, out_dim,kernel_size=3,stride=1, padding=1, bias=False,padding_mode="reflect") # 一样OverlapPatchEmbed
...
# 跟Restormer的TransformerBlock一样代码，只是名字不一样
...
class LocalFeatureExtraction(nn.Module):
    def __init__(self,
        ...
        self.Extraction = nn.Sequential(*[ResBlock(dim,dim) for i in range(num_blocks)]) # 微改把conv2d改成残差
    def forward(self, x):
